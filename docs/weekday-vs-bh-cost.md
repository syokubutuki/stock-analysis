...
## 12. 時変・レグ非対称コストの実装結果（2026-07-31）

### 12.1 実装前の実測（7203.T, 10y）

`/api/stock?ticker=7203.T&range=10y` と同じ修復済み日足、`entry=open` / `exit=close`、窓21日、手数料0、`k=1.5` で測定した。推定点 `j` は `prices[j+1]` まで使うため、約定日 `i` には `j=i-2` を対応させた。

| 推定量 | Cov(v_w, k_w) | σ(k_w) | σ(k_w)/σ(v_w) | Var比 |
|---|---:|---:|---:|---:|
| CS | +8.1×10⁻⁶（+810 bp²） | 0.143% | 7.4% | 0.55% |
| AR | +4.7×10⁻⁶（+470 bp²） | 0.118% | 6.1% | 0.37% |

CS の `Corr(v_w,k_w)` は約0.29。平均コストを定数系列と揃え、時変成分だけを比較した片側 t の変化は約 −1.85→−1.89 相当だった。事前見積もりと同じ桁で、時変化だけで結論は反転しない一方、正の共分散は確認できた。平均コストを全額控除する水準効果はこの −0.04 とは別で、§11 の定数コストと同様に回転率へ比例する。

### 12.2 実装仕様

- `legKind[s]`（entry / exit）と、唯一の課金源 `legCostLog[s]` を導入。週次の経路Aと日次富ループの経路Bは同じ配列だけを引く。
- CS / AR、窓5〜63日（既定21日）をUIで切替可能。`c_i = laggedSpread_i + 2×fee`、entry は `(k/2)ln(1-c_i)`、exit は `(1/2)ln(1-c_i)`。
- B&H の期間先頭買いと末尾売りにも同じ日別系列と非対称係数を適用。
- 時変 Break-even は `λ*=Σv_w/Σk_w` を主表示し、従来の `c*` は定数・対称換算として残した。`λ(95%)`、`Cov(v,k)`、`σ(k)/σ(v)` も表示。
- ブートストラップは控除後の合成系列を再標本化し、gap と cost を分離しない。
- `k` は1〜3の外生スライダー（既定1.5）。既定値に実証的根拠がない感度分析の基準点とUI・ガイドに明記。

### 12.3 §9 受け入れ条件

決定論的な2520営業日のOHLC系列で、変更前HEADと変更後を同じ入力・seedで比較した。

| 条件 | 結果 |
|---|---|
| costRT=0 後方互換 | t / p / Wilcoxon / JKM / 年率差が完全一致（差0） |
| 定数コストの経路A/B | 最大差 1.09×10⁻¹⁴（1e-9以内） |
| 時変CS / ARの経路A/B | 8.88×10⁻¹⁶ / 1.94×10⁻¹⁶ |
| 単調性 | c=0→0.3%で t 1.185→−24.726、Wilcoxon 0.032→−19.144、JKM 0.707→−17.106、年率差 0.67%→−13.00% |
| Δt≈−c√n/σ | 傾きの相対ばらつき 8.44×10⁻¹⁶（1%未満） |
| 非対称感度 | 同じ時変系列で k=1→1.5: t −61.598→−68.344 |

対象4ファイルのESLintは警告なし、`npm run build` は型検査を含め成功。全体 `npm run lint` は対象外の既存箇所（45 errors / 273 warnings）で失敗するため、対象限定lintを合否判定に用いた。デスクトップと390px幅は、通信遮断のため `/api/stock` の決定論的OHLC応答をブラウザで差し替え、時変/定数、CS/AR、窓、k、λ*、チャート描画を確認した。

### 12.4 仕様書からの意図的な変更点

1. `estimateSpread` の出力ラベル `j` は `j+1` の高安を含むため、完全な1日ラグを `i-2` で実現した。`i-1` では約定日 `i` の高安を使ってしまう。
2. 推定前ウォームアップは未来の中央値で埋めず、スプレッド0（手数料は課金）とし、該当レグ数をUIに開示。
3. 時変モデルの主Break-evenを金額 `c*` ではなく系列倍率 `λ*` とした。
4. 非対称係数はスプレッドと手数料を合成した `c_i` 全体へ適用した。入りの追加スリッページを日足から分離推定できないためである。

### 12.5 踏んだ罠

- `SpreadPoint.time` はその日だけで確定する値ではなく、ラベルだけで対応させると前方視になる。
- CSの0クリップで右裾が長くなるため、中央値だけでなくCS/AR切替と `σ(k)/σ(v)` が必要。
- B&H境界を対称のまま残すと、非対称性の取りこぼしが残る。
- 経路別に同じ式を二重実装すると最終手仕舞いと丸めでずれる。`legCostLog[]` を唯一の源にすることが不変量の条件。
