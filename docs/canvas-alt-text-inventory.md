...
**残件は 90 個**（△ の副次図 169 − 79）。理由は「**そのコンポーネントに表があり**、
読み上げが一切届かない状態ではない」ため。**残件0ではない。件数と理由を上に書いた。**

> **この理由が支えている範囲に注意する（2026-09-05 レビューで是正）。**
> §2 の判定はファイル内の `<table` の有無で、支えているのは
> 「**そのファイルに表がある**」までである。**個々の見送り図に対応する表があることまでは示していない。**
> 実際 `SpiralHeatmap.tsx` は見送り10図のうち5図（`domBar` / `dowCumul` / `monthCumul` /
> `yearMonth` / `season`）が**ファイル最後の `<table` より後ろ**にあり、対応する表を持たない。
> 見送りの根拠を図単位で言い切らないこと。
...
残る20ファイルは主図だけを入れて副次図 **59図** を見送った（いずれも**表を持つファイル**である。
図ごとに対応する表があるという意味ではない → §2.1 の注記）。
