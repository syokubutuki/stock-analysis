...
1. **投信名に HTML実体参照が残る。** `03311187` は本番の `name` が
   `eMAXIS Slim米国株式(S&amp;P500)` となり、画面にそのまま出る。
   原因は `stock-source.server.ts:187` の `<title>` フォールバックで、
   HTMLエスケープを解いていないこと。`priceBoard.name` 側の正規表現
   （`yahoo-fund-history.ts:42-44`）はこの銘柄のページ構造に一致せず `null` になる。
...
| FU10 | **価格の有効桁の再検討** | **検証完了・オーナー判断待ち 2026-08-12**（§3.5）。推奨は8桁据え置き。圧縮後の実転送は47.5KBで、6桁化の取り分は8.8KBのみ |
| FU11 | A1 の残46箇所（装飾アイコン＋`app/axioms/page.tsx`） | 未 |
| FU12 | Canvas 内ハードコード灰色 **386箇所** | 未 |
| FU13 | **投信名の HTML実体参照が未デコード**（`&amp;` が画面に出る）。`stock-source.server.ts:187` の `<title>` フォールバック経路のみ。`03311187` で再現 | FU9 由来・未修正 |
