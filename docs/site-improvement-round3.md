...
| FU18 | **U4（品質パネル位置）の初回対処が全節に及んでいなかった**。パネルが `activeSection` の外側にあり18節すべての分析本体の手前に挟まっていた | **完了 2026-08-14**（`233da2f`）。基本節の1か所に集約し、全節共通の開示は `DataQualityNotice` バナーへ集約。目視確認はデプロイ後 |
...
| 2026-08-14 | U4 の積み残し（破損点検パネルが全セクションの最上段を占有）を是正し FU18 として記録。親文書 §11 の U4 行・U4 節にも追記。あわせて `round4.md` §5 にしか無かった FU17（投信で無意味な分析）を親文書 §11 に登録し、番号の重複を解消 |
