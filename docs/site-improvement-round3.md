...
| FU17 | **U4（品質パネル位置）の初回対処が全節に及んでいなかった**。パネルが `activeSection` の外側にあり18節すべての分析本体の手前に挟まっていた | **完了 2026-08-14**（`233da2f`）。基本節の1か所に集約し、全節共通の開示は `DataQualityNotice` バナーへ集約。目視確認はデプロイ後 |
...
| 2026-08-14 | U4 の積み残し（破損点検パネルが全18節の最上段を占有）を是正し FU17 として記録。親文書 §11 の U4 行・U4 節にも追記 |
