...
function drawCurve(canvas: HTMLCanvasElement, r: CapacityResult) {
  ...
  if (r.kStar > 0) marks.push({ k: r.kStar, color: "#059669", label: "K*" });
  if (r.kBreakEven > 0) marks.push({ k: r.kBreakEven, color: "#dc2626", label: "消滅" });
...
export default function EdgeCapacityChart({ prices }: Props) {
  ...
  useEffect(() => {
    ...
    drawCurve(canvasRef.current, selected);
    const onResize = () => { if (canvasRef.current && selected) drawCurve(canvasRef.current, selected); };
    ...
    <div className="space-y-4">
        ...
        <table className="w-full text-xs border-collapse">
          <thead>
            <tr className="text-gray-500 border-b border-gray-200">
              ...
              <th className="text-right px-1">スプレッド後a</th>
            ...
            {table.map((r) => {
              ...
              return (
                ...
                >
                  ...
                  <td className="text-right px-1 font-mono">{(r.muGross * 100).toFixed(3)}%</td>
                  ...
                  <td className={`text-right px-1 font-mono ${r.a > 0 ? "" : "text-red-500"}`}>{(r.a * 100).toFixed(3)}%</td>
                  <td className="text-right px-1.5 font-mono">{dead ? "—" : fmtYen(r.kStar)}</td>
                  <td className="text-right px-1.5 font-mono">{dead ? "0円" : fmtYen(r.kBreakEven)}</td>
                  <td className="text-right px-1.5 font-mono text-amber-700">{fmtYen(r.kLiq)}</td>
                  <td className={`text-right px-1.5 font-mono ${r.profitAtKEff > 0 ? "text-green-700" : "text-gray-400"}`}>{dead ? "—" : fmtYen(r.profitAtKEff)}</td>
                </tr>
              );
            })}
          </tbody>
      ...
      {selected && (
        <div>
          <div className="text-xs text-gray-500 mb-1">
            {selected.edge.label}（{selected.direction === "long" ? "買い" : "売り"}・年{selected.edge.tradesPerYear.toFixed(0)}回）
            — 緑破線=K*(利益最大 {fmtYen(selected.kStar)}) / 赤破線=エッジ消滅({fmtYen(selected.kBreakEven)}) / 橙破線=流動性上限({fmtYen(selected.kLiq)})
          </div>
      ...
      <AnalysisGuide title="エッジ容量推定の詳細理論">
        ...
        <ul className="list-disc pl-4 space-y-1">
          ...
          <li>μは過去平均であり、多重比較補正をしていません。エッジの真偽はスキャン系(FDR)・ウォークフォワード(DSR/PBO)・ヌル較正で先に判定すること。</li>
