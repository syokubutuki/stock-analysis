          ...
          <tbody>
            {res.clusters.map((c) => (
              <tr key={c.id} className={`border-b border-gray-100 ${c.id === res.currentCluster ? "bg-blue-50" : ""}`}>
                ...
                <td className={`text-right px-2 ${c.meanRet >= 0 ? "text-green-700" : "text-red-600"}`}>{(c.meanRet * 100).toFixed(2)}%</td>
                <td className="text-right px-2 text-gray-500">{(c.meanVol * 100).toFixed(1)}%</td>
                <td className={`text-right px-2 font-medium ${c.fwdMean >= 0 ? "text-green-700" : "text-red-600"}`}>{c.fwdMean >= 0 ? "+" : ""}{(c.fwdMean * 100).toFixed(2)}%</td>
              </tr>
            ))}
          </tbody>
        </table>
