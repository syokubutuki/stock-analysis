              ...
              <tbody>
                {result.groups.filter((g) => g.n > 0).map((g) => (
                  <tr key={g.key} className="border-b border-gray-100">
                    ...
                    <td className={`text-right px-2 tabular-nums ${directionClass(g.car0)}`}><DirectionGlyph value={g.car0} />{fmtSignedPct(g.car0)}</td>
                    <td className={`text-right px-2 tabular-nums ${directionClass(g.carPost)}`}><DirectionGlyph value={g.carPost} />{fmtSignedPct(g.carPost)}</td>
                    <td className={`text-right px-2 font-medium tabular-nums ${directionClass(g.postDiff)}`}><DirectionGlyph value={g.postDiff} />{fmtSignedPct(g.postDiff)}</td>
