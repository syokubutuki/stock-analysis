...
export default function TodayBinChart({ prices }: Props) {
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 p-4 space-y-4">
      <div>
        <h3 className="font-bold text-gray-800">今日の値動きが該当するリターンビン：同条件の過去分布（曜日非依存）</h3>
        <p className="text-xs text-gray-500 mt-0.5">
          取得した最新データ（今日の始値など）を前日始値/終値と比べたリターンとして数値化し、過去全履歴のどの分位ビンに該当するかを特定。
          同じビンに入った過去の日が“その後どう動いたか”の分布と期待値を示す。
          これは<b>過去の同条件時の記録</b>であり、将来の値動きの予測ではない。
        </p>
      </div>
      ...
      <AnalysisGuide title="今日の値動きが該当するリターンビンの詳細理論">
        <p className="font-medium text-gray-700">1. 何をしているか</p>
        <p>{"今日の値動きが過去のどのリターン帯（分位ビン）に位置するかを特定し、同じビンに入った過去の日がその後どう動いたかの分布を集計する。曜日や月などのカレンダー要因は一切使わず、純粋に『今日の動きの異常度（分位）』だけを条件にする。これにより全営業日を標本に使え（曜日で絞る分析の約5倍）、10分位や裾の異常値まで統計が安定する。"}</p>
        <p className="mt-1 text-gray-500">{"出力は過去の同条件時の実績分布であり、今エントリーすべきかどうかの判断を示すものではない。標本の期間内でそう見えたという事実に留まる。"}</p>
