...
> ## ⚠ 2026-08-28 更新 — **本書の残件は全部片付いた。運用の正は別文書である**
>
...
> 次に投げるのは **S19（M3 パネルのレジストリ化）。必ず単独で投げること**
> （`app/page.tsx` の構造を組み替えるため並列不可）。**その後に S20（A3 の残り）を単独で。**
> S14 / S16 / S15 / S17 / S18 はマージ済み。本書は設計の記録として残しているだけで、
> **「次に何をやるか」をここから読まないこと。**
>
> 投げ方は `docs/site-improvement-round5.md` §3。プロンプトは生成物なので
> `node docs/tools/build-session-prompts.mjs S19` で作る（`docs/prompts/` を直接編集しない）。
