...
> ## ⚠ 2026-08-21 追記 — **本書の残件は全部片付いた。運用の正は別文書である**
>
> 下の「進捗（2026-08-05 時点）」で未着手だった §4 / §5 は、いずれも完了している。
>
> | 手順 | 現在の状態 |
> |---|---|
> | §4 投信価格アダプターの修復 | **完了**（R0）。`stock-source.server.ts`。退避 catch の絞り込みだけ FU23 に残る |
> | §5 `/t/[ticker]` と SSR サマリー | **完了**（R1・2026-08-15）。98銘柄で公開。取得は 10y・OG画像あり |
>
> **いま進行中の作業は `docs/site-improvement-round5.md` が正。**
> 次に投げるのは **S17（パネル周辺の後始末）と S18（Canvas 代替テキスト）の2本**
> （並列可・第3波）。S14 / S16 / S15 はマージ済み。本書は設計の記録として残しているだけで、
> **「次に何をやるか」をここから読まないこと。**
